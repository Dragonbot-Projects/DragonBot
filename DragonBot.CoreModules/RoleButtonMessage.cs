...
namespace DragonBot.CoreModules
{
    ...
    {
#if DEBUG
        const string COMMAND_NAME = "rbm-test";
#elif RELEASE
        const string COMMAND_NAME = "rbmessage";
#endif
        public static string Name { get; } = "Core_RoleButtonMessage";

        public static RoleButtonMessage Create(Bot bot)
        {
            AsyncContext.Run(() => Logger.Log("Creating RoleButtonMessage module", LogSeverity.Info));
        ...
        {
            if (!bot.Util.IsCommandRegistered(COMMAND_NAME))
            {
                var guild = bot.Client.GetGuild(bot.BotConfig.GuildID);
                SlashCommandBuilder builder = new();
                builder.WithName(COMMAND_NAME)
